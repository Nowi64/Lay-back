...
layback_simulated = np.zeros((len(cable_lengths), len(speeds)))
...
for i, cable_length in enumerate(cable_lengths):
    for j, speed in enumerate(speeds):
        ...
        depth_sim, layback_sim = simulate_for_speed_and_cable(speed, cable_length)

        depth_simulated[i, j] = depth_sim
        layback_simulated[i, j] = layback_sim
        ...
        if layback_ref != 0:
            layback_errors[i, j] = abs(layback_sim - layback_ref) / layback_ref * 100
        else:
...
plt.subplot(1, 2, 1)
sns.heatmap(depth_errors,
            xticklabels=[f'{s} nœuds' for s in speeds],
            yticklabels=[f'{c}m' for c in cable_lengths],
            annot=True,
            fmt='.1f',
            cmap='Reds',
...
plt.tight_layout()
plt.savefig("erreurs_relatives_heatmap.png", dpi=300)
plt.show()
