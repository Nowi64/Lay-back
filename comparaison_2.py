...
for angle in initial_angles:
    result = run_simulation(2.0, 50, angle)
    plt.plot(result['x'], result['y'], label=f'{angle}°', linewidth=2)
plt.xlabel('x (m)')
plt.ylabel('y (m)')
plt.title('Effet angle initial (v=2m/s, L=50m)')
plt.legend()
plt.grid(True)
plt.axis('equal')
...
print("- L'angle initial a un effet modéré sur le profil final")